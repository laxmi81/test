...
public class tc_27 extends page_load {
	...
	public static void main(String[] args) throws Exception {
		...
		Thread.sleep(1000);*/
				...
				Thread.sleep(1000);
				//---//select[@id='hotlist_mode']
				WebElement recent=driver.findElement(By.xpath("//select[@id='hotlist_mode']"));
				recent.click();
				List<WebElement> li_r=driver.findElements(By.xpath("//select[@id='fcf']"));
				for(int i=0;i<li_r.size();i++)
				{
					if(li_r.get(i).getText().contains("Recently created"))
					{
						li_r.get(i).click();
					}
				}
				recent.sendKeys("Recently created");
				recent.click();
				Thread.sleep(1000);
				driver.quit();
